...
export default function PracticeSection({ area, isLast }: PracticeSectionProps) {
  return (
    <section
      id={area.anchor}
      // scroll-mt ensures the section title clears the fixed navbar when linked to
      className={`scroll-mt-28 ${isLast ? "" : "mb-20 pb-20 border-b border-border"}`}
    >
      ...
      <div className="border-l-4 border-blue pl-8">
        ...
        <div className="space-y-10 mb-10 max-w-[760px]">
          {area.body.map((para, i) => (
            <p
              key={i}
              className="font-inter text-base text-text leading-relaxed md:text-justify"
            >
              {para}
            </p>
          ))}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 pt-6 border-t border-border">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-whatsapp text-white font-inter font-medium text-sm hover:opacity-90 transition-opacity"
          >
            <WhatsAppIcon />
            Discuss on WhatsApp →
          </a>
