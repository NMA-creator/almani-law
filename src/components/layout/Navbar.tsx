...
function Logo({ dark = false }: { dark?: boolean }) {
  ...
  const borderColor = dark ? "border-white" : "border-blue";
  return (
    <div className="flex flex-col leading-none select-none">
      <span
        className={`font-inter font-bold tracking-widest text-sm uppercase ${textColor} border-b-2 ${borderColor} pb-0.5`}
      >
        ALI ALMANI
      </span>
      <span
        className={`font-inter font-normal tracking-widest text-xs uppercase ${textColor} mt-0.5`}
      >
        &amp; PARTNERS
      </span>
    </div>
